...
class IMDBImagenet(object):
    ...
    def append_flipped_images(self, roidb):
        ...
        for i in range(self.num_images):
            ...
            oldx1 = boxes[:, 0].copy()
            oldx2 = boxes[:, 2].copy()
            boxes[:, 0] = self.image_sizes[i][0] - oldx2 - 1
            boxes[:, 2] = self.image_sizes[i][0] - oldx1 - 1
            try:
                assert (boxes[:, 2] >= boxes[:, 0]).all()
            except Exception as e:
                # 如果标注的坐标不是0-based，那么在这里会变成65535，导致异常
                print i
                print 'roi'
                print roidb[i]['boxes']
                print 'boxes'
